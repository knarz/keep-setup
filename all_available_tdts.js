...
async function main() {
  try {
    ...
    const tdtContract = new ethers.Contract(
      TBTCDepositToken.networks['1'].address,
      TBTCDepositToken.abi,
      ip
    );
    const vendingContract = new ethers.Contract(
      VendingMachine.networks['1'].address,
      VendingMachine.abi,
      ip
    );

    const transfers = await tdtContract.queryFilter(
      tdtContract.filters.Transfer(null, vendingContract.address)
    );
    const tokenIDs = transfers.map((t) => {
      return t.args[2].toHexString();
    });
    let index = 1;
    for (let addr of tokenIDs) {
      ...
      const vOwns =
        (await tdtContract.ownerOf(d.address)) === vendingContract.address;

      if (depositActive && vOwns) {
        console.log(
          `${index}. TDT ${d.address} (${ethers.utils.formatEther(
            await d.lotSizeTbtc()
          )} tBTC) is active and owned by the vending machine`
        );
        index++;
      }
    }
